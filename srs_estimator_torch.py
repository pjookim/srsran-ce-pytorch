...
def fill_ch_est_cdm(
    ...
    for i in range(n_filled - 1):
        start = filled_list[i] + 1
        stop = filled_list[i + 1] - 1
        stride = stop - start + 1
        if stride <= 0:
            continue

        # span = estimatedAll(stop + 1, :) - estimatedAll(start - 1, :);
        # (stop+1 == 다음 filled index)
        prev = estimated_all[start - 1, :]          # shape (n_layers,)
        nxt = estimated_all[stop + 1, :]            # shape (n_layers,)
        span = nxt - prev

        # estimatedAll(start:stop, :) = prev + span * (1:stride)/(stride+1)
        w = torch.arange(1, stride + 1, device=channel_out.device, dtype=torch.float64) / float(stride + 1)
        w = w.to(dtype=channel_out.real.dtype)  # float32/64 맞추기
        interp = prev.unsqueeze(0) + w.unsqueeze(1) * span.unsqueeze(0)  # (stride, n_layers)
        estimated_all[start : stop + 1, :] = interp
    ...
    for i_layer in range(1, n_layers + 1):
        # MATLAB: estimatedAll(filledIndices(end):end, iLayer) = estimatedAll(filledIndices(end), iLayer);
